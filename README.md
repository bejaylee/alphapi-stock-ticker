...
| 指标 | 结果 |
|------|------|
| FETCH 成功次数 | 882（理论 900，几乎无中断） |
| 心跳 | 60 次（30s 间隔，精确吻合） |
| 中途重启 | 0 次 |
| 疑似冻结 | 0 次 |
| 冻结自愈 | 1 次（开机后约 23s，重启后连续稳定 29.3 分钟） |
| 堆内存 | 稳定在 1973104~1974448 字节，无泄漏 |

结论：2 秒刷新在当前缓解参数组合下可长期稳定运行；偶发的开机期冻结
由看门狗自动重启，约 15 秒内恢复，无需人工干预。
...
- **刷新间隔保持 2s**：30 分钟长跑验证（详见下方稳定性），无需再降频
